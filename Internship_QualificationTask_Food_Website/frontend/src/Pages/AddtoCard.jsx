...
import { ShoppingCart, Trash2, Plus, Minus, ChevronLeft, CreditCard, Clock, MapPin, Shield, ChefHat, Truck, Package, X, Tag } from 'lucide-react';
...
const AddtoCard = () => {
  ...
  const hasAddedRecipe = useRef(false);
  ...
  // Handle incoming recipe from navigation
  useEffect(() => {
    if (recipeFromState && !hasAddedRecipe.current) {
      const addToCart = async () => {
        setIsAdding(true);
        setAddError(null);
        try {
          console.log('Attempting to add to cart:', recipeFromState);
          // ... payload ...
          const payload = {
            recipeId: recipeFromState.id,
            name: recipeFromState.name,
            price: recipeFromState.price,
            image: recipeFromState.image,
            category: recipeFromState.category,
            description: recipeFromState.description,
            quantity: 1,
            selectedSize: 'Regular',
            selectedSpiceLevel: 'Medium', // Default
            selectedAddons: [],
            specialInstructions: ''
          };
          ...
          setAddError(`Failed to add item: ${msg}`);
        } finally {
          setIsAdding(false);
        }
      };
  ...
  // Update size
  const updateSize = async (index, size) => {
    const item = cart[index];
    ...
    } catch (error) { console.error('Error updating size:', error); fetchCart(); }
  };
  
  // Update spice level
  const updateSpiceLevel = async (index, level) => {
    const item = cart[index];
    ...
    } catch (error) { console.error('Error updating spice:', error); fetchCart(); }
  };
  
  // Toggle addon
  const toggleAddon = async (index, addonId) => {
    const item = cart[index];
    ...
    const newCart = [...cart];
    newCart[index].selectedAddons = newAddons;
    setCart(newCart);
    ...
    } catch (error) { console.error('Error updating addons:', error); fetchCart(); }
  };
  
  // Update special instructions
  const updateInstructions = async (index, instructions) => {
    const item = cart[index];
    ...
    // Debounce or just update? For text input usually debounce is better but for now let's update on blur maybe?
    // The current input has onChange calling this.
    // If we call API on every keystroke it's bad. 
    // Let's rely on onBlur in the textarea, but here we can't change the function signature easily without changing JSX.
    // We'll leave the state update here, but maybe NOT call API here?
    // Or we update the API call to be debounced? 
    // Given the task constraints, let's just update local state here and maybe add a save button or onBlur?
    // Actually, looking at the JSX, it uses `onChange`.
    // I will change the logic to ONLY update local state here. 
    // And I will add `onBlur` to the textarea in the JSX to save to DB.
  };

  const saveInstructions = async (index, instructions) => {
    const item = cart[index];
    try {
      await axios.put(`http://localhost:5000/api/cart/${item._id}`, {
        ...item,
  ...
  const handleCheckout = () => {
    ...
    // Clear cart locally (should optimally call backend to clear, but for now UI update)
    setCart([]);
  ...
  // Save cart to localStorage on changes - REMOVED for API
  // useEffect(() => {
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md px-4">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-3">Your cart is empty</h2>
           ...
           {isAdding && (
            <div className="mb-4 text-orange-600 font-semibold animate-pulse">
              Adding item to cart...
            </div>
          )}

          {addError && (
            <div className="mb-4 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative text-sm text-left">
              <strong className="font-bold">Error adding item: </strong>
              <span className="block">{addError}</span>
            </div>
          )}
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-4">
                        ...
                        <textarea
                          ...
                          onBlur={(e) => saveInstructions(index, e.target.value)}
